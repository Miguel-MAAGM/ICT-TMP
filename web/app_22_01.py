...
def add_serial_log(msg):
    """Añade un mensaje al log serial"""
    ...
    while True:
        try:
            ...
            if line.startswith("FOTO"):
                ...
                # 2. Handshake: Avisar a la Pico que puede continuar
...
def send_serial_command(command, wait_for_angle=False, wait_for_ok=False, timeout=3.0):
    """
    Envía comando al Pico y opcionalmente espera respuesta
    - wait_for_angle: espera línea 'ANGULO: xx.xx'
    - wait_for_ok: espera 'OK' o 'ERROR_...'
    """
    global ser
...
def gen_frames():
    ...
    # Imagen placeholder cuando el streaming está pausado
    ...
    while True:
        # Si el streaming está desactivado, mostrar placeholder
...
def control_action(cmd):
    ...
    if cmd == "auto_home":
        ...
        return jsonify({"success": False, "message": "Comando desconocido"})
