...
def loop_auto():
    ...
    while running_loop:
        ...
        #if time.ticks_diff(now, ultima_muestra) > 150:
        ang, raw = leer_angulo()
        #print(f"[INFO]Ángulo={ang:6.2f}°")
        diff=ang - ang_old
        print(f"Angulo Actual: {ang} | Diferencia: {diff} | Raw: {raw}")
        #print(f" Raw= {raw}")
...
def check_captura(ang):
    global ultimo_angulo_foto

    # Primera foto siempre
    if ultimo_angulo_foto is None:
        ultimo_angulo_foto = ang
        print(f"FOTO {ang:.2f}")
        return

    # AS5600 resolución mínima ~0.0879 grados
    if abs(ang - ultimo_angulo_foto) >= 0.07:
        ultimo_angulo_foto = ang
        print(f"FOTO {ang:.2f}")


'''def loop_captura():
    global running_loop, stop_loop, pasos_totales, ultimo_angulo_foto
    ...
    direction = 1
    LOOP_DELAY = 0.0015
    contador_pasos = 0   # <--- NUEVO: control de frecuencia de lectura
    ...
    while running_loop:
        ...
        # Endstop → cambiar dirección
        if stop_loop:
            stop_loop = False
            direction = 0 if direction == 1 else 1

        # ----------- Mover igual que LOOP clásico -------------
        motor.step(1, direction=direction, delay=LOOP_DELAY)

        # -------------------------------------------------------
        #   SOLO cada 3 pasos leo el ángulo (para no romper el timing)
        # -------------------------------------------------------
        contador_pasos += 1
        if contador_pasos >= 3:
            ang, raw = leer_angulo()
        ...
        # NO más sleep aquí (rompe el timing)
'''
...
def loop_captura():
    ...
    while running_loop:
        ...
        # 2) Revisar ENDSTOPS — IGUAL que loop_auto()
        if stop_loop:
            stop_loop = False
            direction = 0 if direction == 1 else 1
            print(f"[INFO] Cambio de dirección → {'DERECHA' if direction==1 else 'IZQUIERDA'}")
            motor._stop_flag = False

        # 3) Mover el motor
        motor.step(1, direction=direction, delay=LOOP_DELAY)

        # 4) Leer siempre el ángulo DESPUÉS DEL PASO
        ang, raw = leer_angulo()

        # 5) Detectar captura cada ~3 pasos (para rendimiento)
        contador_pasos += 1
        if contador_pasos >= 3:
            check_captura(ang)
            contador_pasos = 0

        # NO usar time.sleep grande aquí
